...
var round = function round(number) {
  var toDecimal = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
...
var stayInRange = function stayInRange(value, min, max) {
  var behavior = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'loop';
  // behaviors:
  ...
  if (behavior == 'loop') {
    ...
    var _range = Math.abs(min - max),
        _fullExceed = value < min ? value - min : value - max,
        _quotient = Math.floor(Math.abs(_fullExceed) / _range),
        _exceed = Math.abs(_fullExceed) - _quotient * _range;

    value = value < min && _quotient % 2 === 0 || _quotient % 2 !== 0 ? min + _exceed : max - _exceed;
